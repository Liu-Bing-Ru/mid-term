...
// compare the needs fulfilled and the penalty for adding an extra night shift (if the employee already has night shift it the past 6 days)
		...
		for (int j = 0; j < days; j++) {
			if (workTime[i][j] != 0) {
				...
				if (array[curShift].isnormal == 1) {
					...
					int curTotal = totalNeeds(curShift, j, shift, needs);
/*
...
void fulfillRequest(int holidayRequest[][3100], int workTime[][31], int restDays[], int needs[][24], int shift[][24], int requestNum, int workers, int days, int holidays, int shiftNum, int integers[], int w1)
{
	...
	for (int i = 0; i < requestNum; i ++) {
		if (restDays[holidayRequest[0][i] - 1] >= holidays) {
			int bestShiftChoice = findBestShift(holidayRequest[1][i] - 1, shiftNum, needs, shift, integers);
			int bestTotalNeeds = totalNeeds(bestShiftChoice, holidayRequest[1][i] - 1, shift, needs);
			if (bestTotalNeeds < w1) {
